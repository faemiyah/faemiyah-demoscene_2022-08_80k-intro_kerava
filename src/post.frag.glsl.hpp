...
"void main()"
"{"
"float x=texture2D(T,D).s+.001;"
"vec4 I=texture2D(A,t),H=X(I.q);"
"float T=C(x,H.q*H.q*H.q),t=L(I.stp),D=f(t),X=abs(f(L(texture2D(A,Y).stp))-D),Y=abs(f(L(texture2D(A,E).stp))-D),E=abs(f(L(texture2D(A,Q).stp))-D),Q=abs(f(L(texture2D(A,P).stp))-D),L=max(max(max(X,Y),E),Q),P=smoothstep(.0,.00033,L),f=1.-t;"
"vec3 A=mix(T*vec3(.7,.5,.0)+vec3(.11,.0,.0),vec3(.21,.0,.0),1.-f*f);"
"if(H.t>.0)"
"{"
"vec3 D=C(x,H.t*.7)*H.stp;"
