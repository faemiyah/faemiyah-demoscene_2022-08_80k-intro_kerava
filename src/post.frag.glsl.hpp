...
"float P=x(C,H.q*H.q*H.q),t=L(I.stp),D=f(t),X=abs(f(L(texture2D(A,E).stp))-D),E=abs(f(L(texture2D(A,Q).stp))-D),Q=abs(f(L(texture2D(A,Y).stp))-D),Y=abs(f(L(texture2D(A,T).stp))-D),L=max(max(max(X,E),Q),Y),T=smoothstep(.0,.00033,L),f=1.-t;"
